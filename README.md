...
| POST | `/api/v1/admin/applications/{id}/set-location/` | **[Admin]** Aniq lokatsiyani (lat/long) belgilash |
...
## Muhim eslatmalar
- **Ariza wizard inputlari** (dizayn skrinshotlariga mos validatsiya):
  - Step 1: `business_name`, `category`, `business_type`, `responsible_full_name` — majburiy; `short_description` — ixtiyoriy (max 300 belgi)
  - Step 2: `phone_number` — majburiy, `+998XXXXXXXXX` formatida tekshiriladi; `email`, `instagram`, `telegram`, `website` — ixtiyoriy. `website` protokolsiz (`www.biznes.uz`) ham qabul qilinadi
  - Step 3: `region` — majburiy, O'zbekiston viloyatlari ro'yxatidan tanlanadi (dropdown); `city_district`, `full_address` — majburiy; `work_days`/`work_hours` — ixtiyoriy. **Aniq lokatsiya (lat/long) foydalanuvchi tomonidan kiritilmaydi** — uni operator/admin `admin/applications/{id}/set-location/` orqali alohida belgilaydi
  - Step 4: `discount_percent` — majburiy, 5–100 oralig'ida; `discount_type` — ixtiyoriy (standart: "Barcha mahsulotlar"); agar `discount_type=min_purchase` tanlansa, `min_purchase_amount` majburiy bo'ladi
